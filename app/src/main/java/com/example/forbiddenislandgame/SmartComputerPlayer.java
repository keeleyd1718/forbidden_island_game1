...
public class SmartComputerPlayer extends GameComputerPlayer {
    ...
    protected void receiveInfo(GameInfo info) {
        if (info instanceof FiGameState) {
            ...
            //checking if it is smart ai's turn
            if(this.playerNum == gameState.getPlayerTurn()) {
                //while the smart ai still has actions left
                while(gameState.getActionsRemaining() > 0){

                    if(gameState.areTreasuresCaptured()) {
                        if((gameState.numHelicopterLiftCardsInHand[gameState.getPlayerTurn()] >= 1) &&
                            ...
                            //if the smart ai has at least 1 helicopter lift card and they're also on the FOOLS_LANDING tile the game should be won
                    ...
                    else if(gameState.numWindStatueCardsInHand[gameState.getPlayerTurn()] >= 4){
                        //if the smart ai has 4 wind statue treasure cards
                        ...
                        //if the smart ai has 4 ocean chalice treasure cards
                    ...
                    else if(gameState.numFireCrystalCardsInHand[gameState.getPlayerTurn()] >= 4){
                        //if the smart ai has 4 fire crystal treasure cards
                    ...
                    else if(gameState.numEarthStoneCardsInHand[gameState.getPlayerTurn()] >= 4){
                        //if the smart ai has 4 earth stone treasure cards
                        ...
                        if(gameState.map.get(FiGameState.TileName.FOOLS_LANDING).equals(FiGameState.Value.FLOODED)) {
                            //if fool's landing is flooded shore it up
                            ...
                            //if wind statue treasure tile is flooded shore it up
                            ...
                            //if the ocean chalice treasure tile is flooded shore it up
                            ...
                            //if fire crystal treasure tile is flooded shore it up
                            ...
                            //if earth stone treasure tile is flooded shore it up
                            game.sendAction(new FiShoreUpAction(this, FiGameState.TileName.MOON_TEMPLE));
                        }
                        else if(gameState.map.get(gameState.getPlayerLocation(gameState.getPlayerTurn())).equals(FiGameState.Value.FLOODED)) {
                            //if the tile the player is on is flooded shore it up
